...
df.drop(['classification'], 1, inplace=True)
...
df3dmf = pd.get_dummies(df3, columns=['rbc', 'pc', 'pcc', 'ba', 'htn', 'dm', 'cad', 'appet', 'pe', 'ane'])

if scaleFeatures: df3dmf = scaleFeaturesDF(df3dmf)

pca = PCA(n_components=2, svd_solver='full')
pca.fit(df3dmf)
PCA(copy=True, n_components=2, whiten=False)
T = pca.transform(df3dmf)
df3dmf.shape
...
ax = drawVectors(T, pca.components_, df3dmf.columns.values, plt, scaleFeatures)
T  = pd.DataFrame(T)
